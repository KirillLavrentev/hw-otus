...
func (c *lruCache) Set(key Key, value interface{}) bool {
	...
	if exists && v != nil {
		...
		// если размер очереди больше ёмкости кэша, то необходимо удалить последний элемент из очереди и его значение из словаря).
		if c.queue.Len() > c.capacity {
			...
			delete(c.items, rm_key)        // удалить ключ из словаря.
